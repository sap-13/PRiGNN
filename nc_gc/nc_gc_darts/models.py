...
from llm_search_optimizer import get_pruned_search_space
...
class GNNModel(nn.Module):
    ...
    #################################################################################
    def load_searchspace(self, dataset=None):
        original_search_space = SearchSpace().get_search_space()

        if self.args.use_llm_pruning and dataset is not None:
            if not self.args.gcp_project_id:
                raise ValueError("GCP Project ID is required for LLM pruning. Please set --gcp_project_id.")
            print("INFO: Using LLM to prune the search space...")
            pruned_space = get_pruned_search_space(dataset,
                                                   original_search_space,
                                                   self.args.gcp_project_id,
                                                   self.args.gcp_location,
                                                   self.args.gcp_credentials_path)
            self.SearchSpace = SearchSpace(pruned_space) # Re-initialize with pruned space
        else:
    ...
    def __init__(self, layers, in_features, hidden_features, out_features, args, dataset=None, dropout=0.0):
        ...
        self.load_searchspace(dataset) # Pass dataset to load_searchspace
        self.init_alpha() # Must be after loading search space
        self.load_agg()
        self.update_z()
    ...
    def agg_trans(self, x_n, edge_index, z):
        y = []
        # Only use the aggregators that are in the current (potentially pruned) search space
        if 'sum' in self.search_space['agg']:
            y.append(self.sum_agg(x_n, edge_index))
        if 'mean' in self.search_space['agg']:
            y.append(self.mean_agg(x_n, edge_index))
        if 'max' in self.search_space['agg']:
            y.append(self.max_agg(x_n, edge_index))
